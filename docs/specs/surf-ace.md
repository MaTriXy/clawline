...
5. **Shared screens.** Visit a friend's house, their screens show up. All screens auto-connect.
...
Each screen runs an HTTP server. The provider POSTs to the screen to push content and GETs to read state. The screen POSTs outbound to the provider for events (watch mode, pencil strokes). Both HTTP and HTTPS are accepted — see §15.2.

**Screen HTTP server (provider → screen):**

| Method | Path | Purpose |
|---|---|---|
| `POST` | `/pair` | Create session (auto-connect) |
...
When HTTPS is used, the screen presents its self-signed certificate (derived from its Ed25519 keypair). The provider pins the public key after first pairing. Both HTTP and HTTPS are supported — neither is required, neither is rejected. The protocol in use is determined by the URL.
...
Sessions have **no TTL and no inactivity timeout**. Content stays on screen indefinitely until the provider explicitly clears it or pushes new content. If you're sitting there reading, nothing disappears — ever. There is no timer that will clear the screen.
...
- The screen is shut down or power-cycled.
- The provider explicitly clears the frame and terminates.

There is no explicit disconnect endpoint and no TTL. The provider explicitly clears frames when done. No silent timeouts. No inactivity expiry.
...
When the provider stops sending requests (crash, network blip, iPhone app backgrounded), the screen doesn't immediately go idle. The screen holds the last displayed frame indefinitely — there is no timeout.

For iPhone backgrounding specifically: user switches away from Surf Ace, Surf Ace's HTTP server may become unreachable (iOS suspends it). The provider can't reach the screen. When the user switches back, Surf Ace wakes up, the server comes back online. The provider's next request (frame push or snapshot) succeeds and the session continues. If the provider becomes permanently unreachable (e.g., TARS goes offline), the screen holds the last frame indefinitely. It only goes idle on explicit clear or app quit.
...
When a discovered screen's `pk` matches a trusted entry, the provider auto-connects.
...
1. The screen is shut down.
2. The screen is power-cycled or factory-reset.
3. The provider explicitly clears the frame.

On session end, the screen clears all content, goes idle, and sets `busy=0`.

There is no explicit disconnect endpoint and no inactivity timeout. The provider simply stops sending requests. The provider explicitly clears frames when done. No silent timeouts. No TTL.
...
{
  "callbackUrl": "http://tars.local:18789/surf-ace/events/sf_a1b2c3d4",
  "debounce": {
    "scroll_settle": 500,
    "zoom_settle": 500,
    "text_selected": 0,
...
`POST /watch` subscribes to **all** events — there is no `events` filter field. The screen sends every event type defined in §9.3. Filtering may be added in a future version. The `debounce` object (optional) specifies per-event-type settle delays in milliseconds. Omitting `debounce` or an individual key uses the default (0ms, fire immediately).
...
3. **`surf-ace_pair` action.** CLU invokes this to pair with an untrusted screen (auto mode).
...
**`surf-ace_pair`** — pair with a screen (auto mode):
...
The provider sends `POST /pair { "mode": "auto" }` to the screen and receives a session token. Pairing completes immediately.
...
**Long debounce (~3–5s of no new strokes):** The long debounce timer resets on every new stroke. It fires only after the user stops drawing — pencil lifts or no new strokes for 3–5 seconds (the idle gap). Nothing fires during continuous drawing. On expiry, sends a full surf ace screenshot plus all strokes since the last full snapshot. CLU does a holistic pass — catches cross-surf ace connections, cleans up stale artifacts, fixes anything the short pass missed, resolves ambiguities.
...
|---|---|---|
| iOS | SwiftUI | Phone is a screen. Foreground = broadcasting. Background = HTTP server suspended; session persists, resumes on foreground return. |
...
4. **Handles pairing** — auto-connect acceptance.
...
10. **Manages session** — one active session; session ends only on explicit clear, shutdown, or power-cycle. No inactivity timeout.
...
**Displaying:** Rendering a frame. Responding to GET /snapshot and watch mode events. Content stays until the provider explicitly clears it or pushes new content — no timeout.
...
- Session persists — there is no inactivity timeout.
- User switches back to Surf Ace: HTTP server wakes up. Provider's next request succeeds. Session continues. Provider re-pushes current frame if needed.

The session only ends if the screen is shut down, power-cycled, or the provider explicitly clears it. Background suspension is temporary and recoverable.
...
Auto-connect is used for all screens. After first pairing, the provider trusts the screen's public key. Future connections verify identity via TLS certificate pinning.

Auto-connect is appropriate for home networks. The provider auto-connects to all reachable screens.

### 15.2 Transport

Both HTTP and HTTPS are supported for all Surf Ace communication — between the provider and screens, for event callbacks, and for provider endpoints. HTTP is not rejected. HTTPS is not required. The protocol is determined by the URL in use.

When HTTPS is used, the screen presents its self-signed certificate (derived from its Ed25519 keypair). The provider pins the public key after first pairing — no CA chain needed. This provides encryption and prevents active MITM when TLS is in use.

For home LAN deployments, HTTP is sufficient. HTTPS provides additional security on shared or semi-trusted networks.

### 15.3 Session Tokens

Session tokens are opaque random strings (32 bytes, hex-encoded) generated by the screen on successful pairing. Tokens are valid for the duration of the session (no TTL). Tokens are transmitted over the active transport (TLS-protected when using HTTPS).

### 15.4 Screen Impersonation

A malicious device broadcasting `_surf-ace._tcp` with a spoofed name. Mitigated by:
1. After initial pairing, public key pinning catches identity changes.
2. If a trusted screen's key changes (factory reset or impersonation), the provider flags it and requires re-pairing.
...
The callback URL for watch mode points to the provider (TARS). Events from the screen are POSTed to this URL (HTTP or HTTPS, per the URL provided at watch subscription time). The provider validates that incoming events match an active watch subscription (by source IP and session). Spoofed events from other devices are rejected.
...
TARS restarts. Provider rebuilds screen table from mDNS. Sessions on screens are still active (no TTL — sessions persist until explicitly ended or screen shuts down). Provider re-pairs with trusted screens and can resume pushing content.
...
Screen regenerates keypair. Provider's trusted entry no longer matches the fingerprint in Bonjour TXT. Provider treats it as a new untrusted screen. A new `POST /pair` auto-pairs and re-establishes trust.
...
### 16.11 Rapid Pencil Input (Continuous Drawing)

User draws continuously for 30 seconds without lifting the pencil. **Nothing fires during active drawing.**

- **Short debounce** never fires — it requires the pencil to lift. No lift, no short-debounce event.
- **Long debounce** also does not fire during continuous drawing. The long debounce timer resets on every new stroke. As long as the user is actively drawing (new strokes arriving), the timer never expires. The long debounce fires only after drawing stops: pencil lifts, or no new strokes for 3–5 seconds (the idle gap).

Once drawing stops, the long-debounce timer counts down from 3–5s. If the user resumes before it fires, the timer resets again. When the idle gap is finally sustained, the long debounce fires and sends all accumulated strokes plus the full screenshot.

The surf ace accumulates strokes throughout the entire drawing session and sends them all in the long-debounce payload. CLU may receive a large stroke set. This is fine — the vision model sees the full screenshot and doesn't need to process strokes sequentially.

## 17. Surf Ace API Reference

### 17.1 POST /pair

Create a session with the screen. Always uses auto mode.
...
**Response (success):**
```json
{ "status": "paired", "token": "<sessionToken>" }
```
...
Subscribe to real-time events. Subscribes to **all** event types — no `events` filter field. Optionally includes a `debounce` object with per-event-type settle delays.
