...
- assign built-ins in precedence order: `main`, then `dm` (if present), then `agent:main:main` (if present)
...
All topology mutations are REST. Server then broadcasts WS topology events.
